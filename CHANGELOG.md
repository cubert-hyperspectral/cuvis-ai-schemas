...
- Added the plugin-config contract here so schemas is the single source of truth (cuvis-ai-core drops its fork): `_BasePluginConfig` gains an optional `package_name` author override, and `PluginConfig = GitPluginConfig | LocalPluginConfig` is exported. `provides` carries `CatalogNodeEntry` items directly — the install list and the node catalog are the same list.
