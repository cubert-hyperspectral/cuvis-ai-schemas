...
- **Plugin schema renamed to a descriptive "capabilities" family, one file one manifest (breaking).** Merged `catalog.py`, `plugin/config.py`, and `plugin/manifest.py` into a single `plugin/manifest_capabilities.py` declared in dependency order. Renames: `CatalogPortSpec` -> `NodePortSpec`; `CatalogNodeEntry` -> `PluginCapabilityEntry`; `CatalogPluginEntry` -> `PluginCapabilities` (its `nodes` field -> `capabilities`, `from_manifest_entry(name, dict)` -> `from_manifest(manifest)`); `PluginConfig` (`GitPluginConfig | LocalPluginConfig`) -> `PluginManifest` (`GitPluginSource | LocalPluginSource`); `_BasePluginConfig.provides` -> `capabilities`. The git/local variants are named for the plugin **source** (where it comes from), not a kind of manifest; the union keeps the name `PluginManifest`. The old multi-plugin container `PluginManifest({plugins: dict})` is **eliminated**: one yaml file is now one bare plugin manifest, and `PluginManifest` is the single-plugin union. A new required explicit `_BasePluginManifest.name` field replaces the old dict key and is **never derived from the filename**. Module-level loaders `load_plugin_manifest(path)` (single, resolves a relative local path) and `write_plugin_manifest`; the directory scan + cross-dir duplicate-name guard now lives in cuvis-ai-core's catalog builder rather than in schemas. Hard cut, no shim: old-shape manifests fail validation. On disk, `plugins: {<name>: {..., provides: [...]}}` becomes a bare `name: <name>` + source + `capabilities: [...]`.
- **proto `PluginInfo.provides` -> `capabilities`, and `LoadPlugins` -> singular `LoadPlugin` (regen, breaking).** The FQCN list in the plugin-info response is renamed `provides` -> `capabilities`. The batch RPC `LoadPlugins` becomes singular `LoadPlugin` (`LoadPluginsRequest`/`LoadPluginsResponse` -> `LoadPluginRequest`/`LoadPluginResponse`); a `LoadPlugin` call registers exactly **one** plugin (its `PluginManifest.config_bytes` is a single manifest object, not a list), so callers loop to register several, and the response is singular (`registered_plugin` + `error` instead of the registered/failed maps). This restores the one-JSON-Pydantic-schema-per-`config_bytes` convention. `InitializeSession.resolved_plugins_json` is unchanged: still a JSON **list** carrying the resolved plugin set to the child runtime. Stubs regenerated with `buf generate`.
...
- **`PipelineMetadata.cuvis_ai_version` auto-stamps the installed version.** The field defaulted to a static `"0.1.0"` literal (and `cuvis_ai_schemas.__version__` was hardcoded to the same), so every freshly serialized pipeline recorded `0.1.0` regardless of the real version. `__version__` now derives from the installed distribution (`importlib.metadata`, setuptools-scm), and the field's `default_factory` stamps it; a value already recorded (e.g. by an older snapshot) is preserved on load.
