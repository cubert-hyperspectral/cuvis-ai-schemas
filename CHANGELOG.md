...
- **Plugin schema renamed to a descriptive "capabilities" family, one file one manifest (breaking).** Merged `catalog.py`, `plugin/config.py`, and `plugin/manifest.py` into a single `plugin/manifest_capabilities.py` declared in dependency order. Renames: `CatalogPortSpec` -> `NodePortSpec`; `CatalogNodeEntry` -> `PluginCapabilityEntry`; `CatalogPluginEntry` -> `PluginCapabilities` (its `nodes` field -> `capabilities`, `from_manifest_entry(name, dict)` -> `from_manifest(manifest)`); `PluginConfig` (`GitPluginConfig | LocalPluginConfig`) -> `PluginManifest` (`GitPluginManifest | LocalPluginManifest`); `_BasePluginConfig.provides` -> `capabilities`. The old multi-plugin container `PluginManifest({plugins: dict})` is **eliminated**: one yaml file is now one bare plugin manifest, and `PluginManifest` is the single-plugin union. A new required explicit `_BasePluginManifest.name` field replaces the old dict key and is **never derived from the filename**. New module-level loaders `load_plugin_manifest(path)` (single) and `load_plugin_manifests(dirs)` (directory scan that builds a `name -> path` index and **errors on a duplicate name across all dirs**, returning `list[PluginManifest]`) plus `write_plugin_manifest`. Hard cut, no shim: old-shape manifests fail validation. On disk, `plugins: {<name>: {..., provides: [...]}}` becomes a bare `name: <name>` + source + `capabilities: [...]`.
- **proto `PluginInfo.provides` -> `capabilities` (regen).** The only proto field change: the FQCN list in the plugin-info response is renamed; stubs regenerated with `buf generate`. `LoadPluginsRequest` is unchanged: its `PluginManifest.config_bytes` JSON now carries a **list** of single-plugin manifests instead of a `{plugins: {...}}` map (no `repeated` field, no version envelope; an old-shape payload fails with a raw pydantic error). The `InitializeSession.resolved_plugins_json` doc comment now describes the list-of-manifests shape.
