...
- **Plugin schema reworked into a "capabilities" family; one yaml is one manifest (breaking).** `catalog.py`, `plugin/config.py`, and `plugin/manifest.py` collapse into a single `plugin/manifest_capabilities.py`. Renames: `CatalogPortSpec` -> `NodePortSpec`; `CatalogNodeEntry` -> `PluginCapabilityEntry`; `CatalogPluginEntry` -> `PluginCapabilities`; `PluginConfig` -> `PluginManifest`, with source variants `GitPluginSource` / `LocalPluginSource` (named for where the plugin comes from); `provides` -> `capabilities`. A manifest is now one bare plugin with a required explicit `name` (never derived from the filename), not a `{plugins: {<name>: ...}}` map. Module loaders are `load_plugin_manifest` (single) + `write_plugin_manifest`; the plural directory loader was removed. Hard cut: old-shape manifests fail validation.
- **`PluginCapabilityEntry` gains `kind` / `data_module_name` / `extras`.** `kind: node | data_module` (default `node`); a `data_module` entry declares a globally-unique `data_module_name` and may list pip `extras`; a `node` entry sets neither. Existing manifests omit all three and stay nodes.
- **proto: `LoadPlugins` becomes singular `LoadPlugin`; `PluginInfo.provides` -> `capabilities` (regen, breaking).** One `LoadPlugin` registers one plugin (`config_bytes` is a single manifest object, not a list); the response is singular (`registered_plugin` + `error`). Callers loop to register several. `InitializeSession.resolved_plugins_json` stays a JSON list.

### Data layer

- **`DataConfig` is module-agnostic (breaking).** The cu3s-specific fields (`cu3s_file_path`, `annotation_json_path`, `train_ids` / `val_ids` / `test_ids`, `train_split` / `val_split`, `shuffle`, `processing_mode`) are replaced by `{data_module, splits, batch_size, num_workers, params}`. `data_module` selects a registered DataModule by its `DATA_MODULE_NAME`; module-specific arguments ride in `params`.
- **`DataSplitConfig` uses composable selectors over an attributed sample universe (breaking).** Now `{splits_path, leakage_check, universe_hash, train/val/test/predict: list[Selector]}`. Added `SampleRef` (a universe element with a content-derived `uid`), `SelectorKind` (`files / file_indices / dir_indices / stems / glob / tag / categories / all / union / except / intersect`), and `Selector` (structure-only validator; category/tag names checked at setup, not parse). Stages union in order; empty `predict` means all samples. `leakage_check` (`error` default / `warn` / `off`) guards train/val/test disjointness at setup. Replaces the flat `train_ids/val_ids/test_ids/predict_ids` id-lists.

### Training and pipeline

- **`TrainRunConfig.pipeline` is a path reference, not an inline config (breaking).** Field changed `PipelineConfig | None` -> `str | None`: a path to the pipeline YAML, resolved relative to the trainrun file (a bare name resolves on the pipeline search path). Inline mappings and Hydra `@pipeline` compositions are rejected with a fix-it hint.
- **`LoadPipelineRequest.data_module` (proto, additive).** Optional `string data_module = 3`, the data-module name, so the orchestrator resolves that module's plugin and pip extras when it composes the child env at `LoadPipeline`. Carries only the name, not a `DataConfig`.
- **`PipelineMetadata.cuvis_ai_version` auto-stamps the installed version.** It was a hardcoded `"0.1.0"`, so every serialized pipeline recorded `0.1.0`. `__version__` now derives from the installed distribution (`importlib.metadata` / setuptools-scm) and the field's `default_factory` stamps it; a value already recorded is preserved on load.
