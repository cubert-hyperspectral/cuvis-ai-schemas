...
- Added a module-level `_ORCHESTRATION_FIELDS` frozenset in `cuvis_ai_schemas.training.config` naming every `TrainingConfig` field that is not a `pl.Trainer` keyword (`seed`, `optimizer`, `scheduler`, `callbacks`, `release_cuda_cache_on_validation`). Together with `_LIGHTNING_FIELDS` it partitions the model's fields, and the test suite asserts that partition (union equals `model_fields`, intersection empty), so a new field has to be filed on exactly one side. The training config still rides as JSON inside the unchanged `TrainingConfig` proto message; no stub regeneration and no wire change.
...
- Bumped CI actions: `actions/checkout` v6 → v7, `actions/setup-python` v5 → v6, `codecov/codecov-action` v6 → v7. CI-only; no package or schema changes.
